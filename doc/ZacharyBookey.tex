...
optimizations in the future by exploiting heirarchial parallelism. For example, in the inexact
...
near identical method can be applied to our level solve algorithm but since the level solve deals
with fewer rows at a time we won't see as much of a performance increase in here. We're confident
that these aren't the only places that can benefit from heirarchial parallelism and we plan to
