...
An example of "nested loop to Kokkos  kernel" conversion is presented in the Figure \ref{Kokkos_kernel}.
Here we replace outer loop with the \texttt{Kokkos::parallel\_for} and put internal part of the loop to the Kokkos kernel (see right  part of the Figure \ref{Kokkos_kernel}).
...
\begin{figure}[h]
\subfloat[Simple SYMGS]{\includegraphics[width = 4.3cm, height=3.3cm]{plots/ZAB-VanillaSYMGS.png}}
\subfloat[Level Solve]{\includegraphics[width = 4.3cm, height=3.3cm]{plots/ZAB-LevelSolve.png}}
\subfloat[Inexact Solve]{\includegraphics[width = 4.3cm, height=3.3cm]{plots/ZAB-InexactSolve.png}}
...
If we start by looking at our original LowerTrisolve kernel used in the inexact solve algorithm (see Figure \ref{Kokkos_kernel}) 
